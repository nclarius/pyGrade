...
class Tree(object):
    ...
    def preorder(self):
        ...
        Inorder: left child first, parent, then other children
        :return: a list of tree nodes in pre-order
        :rtype list[Tree]"""
        result = []
        if len(self.children) > 0:
            result += self.children[0].preorder()
        result.append(self)
    ...
    def postorder(self):
        ...
        children from left to right, then parent
    ...
    def is_xbar(self):
        ...
        :rtype bool"""
        if self.is_phrase_projection() and self.is_bar_projection() and self.is_head_projection() or self.is_terminal() is True:
            return True
        else:
            return False
    ...
    def is_phrase_projection(self):
        ...
        return (self.level == 2 and
                (self.is_no_specifier_structure() or self.children[0].is_specifier_structure()))
    ...
    def is_bar_projection(self):
        ...
        return (self.level == 1 and
                (self.is_no_adjunct_or_complement_structure() or self.is_adjunct_structure() or self.is_complement_structure()))
